...
def get_quality_summary():
    return session.sql("""
        SELECT
            ...
            quality_score,
            total_issues
        FROM SNOWFLAKE_EXAMPLE.DATAQUALITY_METRICS.DT_QUALITY_SUMMARY
    """).to_pandas()
...
def get_market_quality():
    return session.sql("""
        ...
        FROM SNOWFLAKE_EXAMPLE.DATAQUALITY_METRICS.DT_MARKET_TRENDS
...
try:
    ...
    st.subheader("Quality by Market Area")
    ...
    st.markdown("""
1. **Verify deployment completed:** Run `SHOW DYNAMIC TABLES IN SCHEMA SNOWFLAKE_EXAMPLE.DATAQUALITY_METRICS;`
