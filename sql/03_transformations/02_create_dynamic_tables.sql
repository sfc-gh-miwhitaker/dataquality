 ...
 *
 * OBJECTS CREATED:
 *   - DATAQUALITY_METRICS.DT_QUALITY_SUMMARY
 *   - DATAQUALITY_METRICS.DT_MARKET_TRENDS
 *
...
CREATE OR REPLACE DYNAMIC TABLE DT_QUALITY_SUMMARY
    TARGET_LAG = '1 minute'
    WAREHOUSE = SFE_DATAQUALITY_WH
    REFRESH_MODE = AUTO
    INITIALIZE = ON_CREATE
    COMMENT = 'DEMO: Real-time quality metrics with 1-minute lag | Author: SE Community | Expires: 2026-02-05'
AS
SELECT
...
CREATE OR REPLACE DYNAMIC TABLE DT_MARKET_TRENDS
    TARGET_LAG = '5 minutes'
    WAREHOUSE = SFE_DATAQUALITY_WH
    REFRESH_MODE = AUTO
    INITIALIZE = ON_CREATE
    COMMENT = 'DEMO: Real-time market trends with 5-minute lag | Author: SE Community | Expires: 2026-02-05'
AS
SELECT
...
ALTER DYNAMIC TABLE DT_QUALITY_SUMMARY REFRESH;
ALTER DYNAMIC TABLE DT_MARKET_TRENDS REFRESH;
...
-- To verify manually: SHOW DYNAMIC TABLES IN SCHEMA SNOWFLAKE_EXAMPLE.DATAQUALITY_METRICS;

SELECT
    'Dynamic Tables Created & Refreshed' AS status,
    'DT_QUALITY_SUMMARY (1 min lag), DT_MARKET_TRENDS (5 min lag)' AS tables_created,
